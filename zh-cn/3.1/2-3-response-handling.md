...
API 返回均是响应 `json`, 因为主要单独针对 API 场景, 如果功能无法满足业务需要，可以通过自定义 [扩展](zh-cn/3.1/3-8-extend.md) 来实现业务目的，但必须返回 `Psr\Http\Message\ResponseInterface` 抽象接口类。
...
服务器基于 Swoole 扩展实现，具体可以前往: [Swoole服务器](zh-cn/3.1/3-9-swoole-server.md)

下一节: [授权](zh-cn/3.1/2-4-authorization.md)